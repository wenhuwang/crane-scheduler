...
func (ds *DynamicScheduler) Score(ctx context.Context, state *framework.CycleState, p *v1.Pod, nodeName string) (int64, *framework.Status) {
	...
	for _, hv := range ds.schedulerPolicy.Spec.HotValue {
		hotValue += ds.bindingRecords.GetLastNodeBindingCount(nodeName, hv.TimeRange.Duration) / hv.Count
	}
	...
	klog.V(5).Infof("Plugin[%s] Node[%s]'s final score is %d, while score is %d and hotValue is %d", ds.Name(), node.Name, finalScore, score, hotValue)
